...
export default function App() {
    ...
    function handleSaveArticle(
        data: Omit<Article, "id" | "createdAt" | "wearCount" | "lastWornAt">
    ) {
        const currentScreen = screen;
        const existingId =
            currentScreen.name === "add-article"
                ? currentScreen.articleId
                : undefined;
    ...
    function handleSaveOutfit(data: {
        name: string;
        occasions: Occasion[];
        articles: { articleId: string; layer: Layer }[];
    }) {
        const currentScreen = screen;
        const existingId =
            currentScreen.name === "outfit-builder"
                ? currentScreen.outfitId
                : undefined;
        const existingOutfit = existingId
            ? OutfitRepository.getById(existingId)
            : undefined;
        const saved = OutfitRepository.save(
            existingId
                ? {
                      ...data,
                      id: existingId,
                      createdAt: existingOutfit?.createdAt,
                      wearCount: existingOutfit?.wearCount,
                      lastWornAt: existingOutfit?.lastWornAt,
                  }
                : data
        );
        refresh();
    ...
    function renderScreen() {
        switch (screen.name) {
            case "closet-home":
                return (
                    <ClosetHomeScreen
                        ...
                        onArticleClick={(id) =>
                            navigate({ name: "article-detail", articleId: id })
                        }
                    />
                );

            case "add-article": {
                const existing = screen.articleId
                    ? ArticleRepository.getById(screen.articleId)
                    : undefined;
                return (
                    <AddEditArticleScreen
                        ...
                        onDelete={
                            screen.articleId ? handleDeleteArticle : undefined
                        }
                        onBack={goBack}
                    />
                );
            ...
            case "article-detail": {
                ...
                const space = article.storageSpaceId
                    ? StorageSpaceRepository.getById(article.storageSpaceId)
                    : undefined;
                return (
                    <ArticleDetailScreen
                        article={article}
            ...
            case "storage-spaces":
                return (
                    <StorageSpacesScreen
                        ...
                        onUnassigned={() =>
                            navigate({ name: "unassigned-articles" })
                        }
                    />
                );
            ...
            case "unassigned-articles": {
                ...
                const virtualSpace: StorageSpace = {
                    id: "",
                    name: "Unassigned Items",
                    subLocation: "No storage space",
                    createdAt: "",
                };
                return (
                    <StorageSpaceDetailScreen
            ...
            case "outfits-list":
                return (
                    <OutfitsListScreen
                        ...
                        articles={articles}
                        onOutfitClick={(id) =>
                            navigate({ name: "outfit-detail", outfitId: id })
                        }
            ...
            case "outfit-builder": {
                const existing = screen.outfitId
                    ? OutfitRepository.getById(screen.outfitId)
                    : undefined;
                return (
                    <OutfitBuilderScreen
                        outfit={existing}
            ...
            case "outfit-detail": {
                ...
                return (
                    <OutfitDetailScreen
                        ...
                        onLocateArticle={(spaceId) =>
                            navigate({ name: "storage-space-detail", spaceId })
                        }
                        wearHistoryEnabled={settings.wearHistoryEnabled}
